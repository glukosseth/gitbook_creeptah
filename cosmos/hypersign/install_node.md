...
echo "export DAEMON_HOME=$HOME/.hid-node >> ~/.bash_profile
...
unzip -o genesis.zip -d $HOME/.hid-node/config/
rm genesis.zip
```
### Config app
...
```Bash
seeds=""
sed -i "s/^seeds *=.*/seeds = \"$seeds\"/;" $HOME/.hid-node/config/config.toml
peers=""
...
Set min gas price
```Bash
sed -i -e "s/^minimum-gas-prices *=.*/minimum-gas-prices = \"0.02uhid\"/" $HOME/.hid-node/config/app.toml
```
Enable/Disable snapshot (optional)
```Bash
snapshot_interval="1000" && \
sed -i.bak -e "s/^snapshot-interval *=.*/snapshot-interval = \"$snapshot_interval\"/" ~/.hid-node/config/app.toml
```
...
sudo rm /lib/systemd/system/hided* -rf
