...
##### Transfer funds
```Bash
strided tx bank send <wallet_address> <to_wallet_address> 10000000ustrd --chain-id STRIDE-TESTNET-4 -y
```
##### Voting
```Bash
strided tx gov vote <number_proposal> <yes/no> --from <wallet_name> --chain-id=STRIDE-TESTNET-4
```
##### Delegate stake
```Bash
strided tx staking delegate <valoper_address> 10000000ustrd --from=<wallet_name> --chain-id=STRIDE-TESTNET-4
```
##### Withdraw all rewards
```Bash
strided tx distribution withdraw-all-rewards --from=<wallet_name> --chain-id=STRIDE-TESTNET-4
```
##### Withdraw rewards with commision
```Bash
strided tx distribution withdraw-rewards <valoper_address> --from=<wallet_name> --commission --chain-id=STRIDE-TESTNET-4
```
##### Edit validator
```Bash
